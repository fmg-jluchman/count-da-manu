...
\section{Introduction}
	...
	The conceptual value of behavior counts as outcomes aside, behavior counts introduce new challenges when used in data analysis as a dependent variable/DV. 
	Despite being ostensibly continuous in nature (i.e., by comparison to Likert-type scales on surveys), behavior counts introduce analytic challenges due to the discrete values and truncated, non-negative (i.e., $\geq$ 0) range on which they take.
	The discreteness and truncated range taken by typical count DVs tend to result in substantial deviations from the Normal distribution. 
	When the distribution of count DVs substantially deviates from Normally distributed, using the commonly-applied linear regression model/LRM is inadvisable \cite(e.g.,){blevins2015count}.
	
	A better data analysis strategy with behavior counts uses regression models designed for non-negative integer (i.e., count) distributions.
	For instance, the Poisson regression/PR and negative Binomial regression/NBR models are count regression models/CRMs commonly used in the literature and implemented in many analytic software environments.	
...
%	\begin{equation}
%		C_{IV_x}^{s} = \sum_{l=1}^{C_{U_l \cup IV_x}} \frac{ R^2_{U_l \cup IV_x} - R^2_{U_l}}{\pi_{U_l \cup IV_x}}
%	\end{equation}
