...
\section{Introduction}
	...
	Organizational science often focuses on exhibition of specific behaviors as a key outcome of interest. 
	These behaviors can be at the organization-level such as the number of organizations adopting a specific practice in a week \cite{naumovska2021strength} or number of divestitures organizations make in a year \cite{bettinazzi2021stakeholder} or at the individual-level such as the number of scientific articles published in a year by an academic \cite{rotolo2013does} or number of errors that resulted in an accident in the last three months among medical doctors \cite{naveh2015active}.
	Behaviors such as these are of great value to organizational science as they are typically have strong construct validity in representing the intended outcome domain and are commonly used as indications of criterion-oriented validity \cite(e.g.,){cronbach1955construct}.
	Despite their conceptual utility, behaviors such as these produce counts of discrete, usually infrequent, events that present challenges for data analysis when used as a dependent variable/DV.
	The challenge introduced for data analysis by using a count DV is that the distribution of such data can show substantial violations of Normality and, consequently, make the application of the linear regression model/LRM inadvisable \cite(e.g.,){blevins2015count}.
...
%	are commonly used as dependent variables/DVs in organizational science \cite(e.g.,){bettinazzi2021stakeholder,naumovska2021strength,soda2021networks} and are modeled using 
	When analyzing data with count DVs, it is advisable to use models designed to accommodate the unique features of these count variable distributions. 
	These, count regression, models/CRMs include Poisson regression/PR and negative Binomial regression/NBR.  
	
	...ended here...
	
	CRMs differ from the commonly used linear regression model/LRM as their functional forms are exponential or log-linear (i.e., have the form $e^{\sum\beta x}$) to accommodate the necessarily truncated (i.e., non-negative) range of the DV.	
...
\section{Dominance Analysis}
	...
	The use of predicted versus observed differences is a form of variance-based importance and has a long history in organizational and behavioral science as a method for inferring importance \cite{johnson2004history}.
	What makes DA unique among variance-based importance methods is DA's conceptual foundation as an extension of Shapley value decomposition from cooperative game theory \cite(e.g.,){winter2002shapley}.
