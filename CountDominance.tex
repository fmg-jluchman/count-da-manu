...
\section{Introduction}
	...
	Organizational science often uses how many times a specific behavior is observed as an outcome of interest to answer research questions. 
	Behavior counts as outcomes arise at the organization-level such as when measuring the number of organizations adopting a specific practice in a week \cite{naumovska2021strength} or number of divestitures organizations make in a year \cite{bettinazzi2021stakeholder}. Behavior counts as outcomes also arise at the individual-level such as when measuring the number of scientific articles published in a year by an academic \cite{rotolo2013does} or number of errors that resulted in an accident in the last three months among medical doctors \cite{naveh2015active}.
	...
	The conceptual value of behavior counts as outcomes aside, such variables are challenging for data analysis when used as a dependent variable/DV. 
	Behavior counts are discrete, usually infrequent, numbers of behavior events that show substantial deviation from the Normal distribution. 
	Consequently, analyzing count DVs with the linear regression model/LRM is inadvisable \cite(e.g.,){blevins2015count}.
%	are oftentimes useful, informative data when used as an outcome
%	are commonly used as dependent variables/DVs in organizational science \cite(e.g.,){bettinazzi2021stakeholder,naumovska2021strength,soda2021networks} and are modeled using 
	
	A better data analysis strategy with behavior counts uses count regression models/CRMs designed for non-negative integer (i.e., count) distributions.
	For instance, the Poisson regression/PR and negative Binomial regression/NBR models are CRMs commonly used in the literature and implemented in many analytic software environments.	
	CRMs such as these are exponential or log-linear (i.e., have the form $e^{\sum\beta x}$) models that are effective for modeling the truncated, non-negative ranges of count DVs.
	
	A log-linear CRM coefficient is challenging to interpret directly as it describes logarithmic change to the DV given a 1 unit change to an independent variable/IV. 
	When translated through an exponential function, a CRM coefficient is known as an incidence rate ratio and describes the percentage change in the count DV per unit change to the IV.
	Percentage changes associated with CRM coefficients make evaluating model-to-data fit more challenging as well.
	The explained variance $R^2$, a standard fit metric for LRM, is not as effective a statistic for describing the quality of model predictions' fit to the data with CRMs \cite{cameron1996r}.
	
	The ineffectiveness of the explained variance $R^2$ for CRM also implies that standard model post-estimation methods using the fit statistic, including relative importance analysis methods like dominance analysis/DA \cite{azen2003dominance}, do not apply to CRMs as they do with LRM.
	DA is a widely used relative importance analysis method that is applied to assist in the interpretation of model results \cite{tonidandel2011relative}.  
	DA was developed for use with LRM and has been extended in published work to generalized linear models including  binary \cite{azen2009using}, ordered, and multinomial logit models \cite{luchman2014relative}.
	To date, no published work has similarly extended DA to CRMs.
...
% reshape this section to discuss DA in descending levels of stringency then tie GDS with Shapley and extend to CRM
	DA is a relative importance analysis method for LRM developed by Budescu \citeyear{budescu1993dominance} that extended on past methods by defining IV importance in terms of pairwise IV comparisons that involve three different, and increasingly stringent, levels of "dominance."	
	The strongest of the dominance designations is called \emph{complete dominance}. 
	Complete dominance is determined between two IVs, for instance $IV_x$ and $IV_z$, by comparing the amount of variance each explains or the $R^2$ associated with each.
