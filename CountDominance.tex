...
\section{Introduction}
	...
	This work first reviews DA as a relative importance methodology provides an accessible discussion of the relationship between DA and Shapley value decomposition.
	...
	Statistical models are analogous to cooperative games and, in highlighting the relationship between these two methodologies, this work demonstrates the applicability of DA to CRMs as well as other statistical models and fit statistics that are used describe those models.
	In addition, the analogy between statistical models and cooperative games is useful as a way to think about how importance is determined and can make the abstract concepts discussed in this work more concrete.
	
	Second, this work recommends using a specific pseudo-$R^2$ statistic for CRM-based DA and implements multiple data analytic examples of DA with that statistic.
	...
	Third, this work discusses data generation mechanisms with count DVs and outlines how research questions with count DVs can, under certain circumstances, be affected by a concept known as differential exposure.
	...
	I end this first section by outlining DA's relationship to Shapley value decomposition.
	In discussing DA's relationship with Shapley value decomposition, I more clearly show the broad applicability of DA as a methodology to understand predictions in statistical analysis and the potential applicability of methods from allied literatures to common problems in DA.
...
\section{Dominance Analysis: A Unique Relative Importance Analysis Method}
	...
	Research methodologists and statisticians have used many methods over the years to determine how important an IV is in linear regression \cite(see reviews in){gromping2007estimators, johnson2004history}.
	...
	DA was originally developed as a method for determining IV importance for linear regression by Budescu \citeyear{budescu1993dominance} that extended on past methods by defining importance in terms of pairwise IV comparisons as inferred from the direct comparison of sub-model $R^2$ values.
	...
	As an example of how the dominance comparison is implemented, consider a model with 4 IVs: $IV_x$, $IV_z$, $IV_w$ and $IV_v$.
	If the dominance comparison was focusing on $IV_x$ versus $IV_z$, there would be a total of 3 possible $R^2$ comparisons: $R^2_{Y \sim IV_x + IV_w}$ versus $R^2_{Y \sim IV_z + IV_w}$; $R^2_{Y \sim IV_x + IV_v}$ versus $R^2_{Y \sim IV_z + IV_w}$; and $R^2_{Y \sim IV_x}$ versus $R^2_{Y \sim IV_z}$; where the subscripted model indicates the form of the linear regression equation in symbolic form (i.e., similar to an R formula interface) and $Y$ is the DV.
	$IV_x$ dominates $IV_z$ only when all the sub-model $R^2$ values that include $IV_x$ are greater than the sub-model $R^2$ values that include $IV_z$'s values across all three of these sub-models.
	...
	The reason this designation is so difficult to achieve is non-compensatory; all sub-model $R^2$ value comparisons must favor one IV over another or the designation will fail to be achieved.
	I also offer the reader an alternative interpretation of complete domiance that I believe is more intuitive but does require some explanation and a re-thinking of what a statistical model is does.
	
	Imagine, if you will, that linear regression is actually a kind of matching game.
	The objective of the game for the players to work together to devise a set of numbers for themselves and their teammates such that they match their game piece values to the goal pieces' values subject to a specific rule.
	The goal piece values in this case are those of the DV across all observations.
	The game piece values for the players are similarly the IVs values across all observations.
	The specific rule to which the players must adhere is that the numbers divised must have squared deviations from the goal pieces that sum to the smallest possible value.
	The numbers the players assign to themselves in this case are the linear regression coefficients.
	
	... ended review here ...
	
	devise a single number for themselves
	
	In this matching game, the observations are game pieces, and the  number such that, when this single number is 
	
	The IVs in the model are players in this game.
	The IVs play this game by taking their pieces,
	
	
	columns of a dataset are not IVs but players in a game and the data represented by them in rows are 'information' each knows about the DV
	
	omplete dominance can also be interpreted 
	...
	... expand here and tie into complete dominance... ask reader to consider this a competition ...
	...
	These comparisons have been formalized into three different levels of dominance between IVs \cite{azen2003dominance} where each level of dominance has a different implied strength or level of stringency. 
	I interpret these stringency levels as the extent to which the comparisons between the IVs must be in order to determine that one IV, for example $IV_x$, is dominates another IV, for example $IV_z$.
	
	The pairwise comparisons bet
	
	At this point I ask the reader to change their frame of mind about what DA is and how it is computed.
	...
	Given the general dominance designation process involves a single comparison, the IV pair being compared can almost always designate general dominance of one IV over another.
	Thus, general dominance, as a designation between an IV pair, is likely to be achieved.
	An additional unique feature of the general dominance statistics used to determine general dominance is that they sum to the value of the $R^2$ across all IVs.
	Thus, general dominance statistics is the part of the $R^2$ attributable to an IV. 
	It is this last, part of the whole, interpretation that links general dominance statistics to another well-known methodology 
	...
	Although it is a popular method, DA is not the only relative importance methodology to evaluate IV contributions to the explained variance $R^2$ and, indeed, there are a number of similar metrics that have been used in organizational and behavioral science as a method for determining importance \cite(see)[for a review]{johnson2004history}.
	An additional, and unique, advantage of DA is the extensibility of the methodology given its relationship to a well-known, methodology from game theory for ascribing payoffs in a game to players: Shapley value decomposition \cite(e.g.,){winter2002shapley}.
	
	The relationship between DA and Shapley value decomposition is important and infrequently acknowledged in published research \cite(see)[for a rare exception]{gromping2007estimators}.
	The relationship between these methods is important as Shapley value decomposition has a rigorous conceptual foundation and is a broadly applicable method for ascribing values to players in a cooperative game.
	Therefore, the analogy between DA and Shapley values can justify DA's extension to new 
	
	Cooperative games can be thought of as a structured form of interaction in which the interacting parties are required to work together toward a common goal and share information with one another \cite(e.g.,){elkind2016cooperative}.
	...
	$$
		\delta_{IV_x} = 
		\frac{\sum_{i=1}^{\pi} R^2_{O_i + IV_x} - R^2_{O_i}}{\pi} = 
		\frac{\sum_{j=1}^{p} \sum_{l=1}^{\phi_j} R^2_{U_{jl} + IV_x} - R^2_{U_{jl}}}{p*\phi_j}
	$$

	Where $\pi$ refers to the total number of permutations of the $p$ players and $O_i$ is some distinct ordered set (i.e., where the order of inclusion matters) of players not including $Pl_a$ that can include the null set of no players.
