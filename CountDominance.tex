...
\section{Introduction}
	...
	This work first reviews DA as a relative importance methodology and provides an accessible discussion of the relationship between DA and Shapley value decomposition.
	...
	%revisit #1 here to be sure I do it as described
...
\section{Dominance Analysis: A Unique Relative Importance Analysis Method}
	...
	In each comparison, the subscripted model indicates the form of the linear regression equation in symbolic form (i.e., similar to an R formula interface).
	...
	One alternative way of looking at the process of determining complete dominance is to see it as an experiment conducted on the predictive model.
	Specifically, if the inclusion or exclusion of each IV in the full model is considered a factor in this experiment, conditional dominance is the process of comparing the values simple effects for $IV_x$ to those for $IV_z$ across the full-factorial of levels of the other $p -2$ IVs.	
	
	The process for determining complete dominance between two IVs, $IV_x$ and $IV_z$ with an arbitrary number of IVs in the model ($p$) can also be defined symbolically as:
	
	\begin{equation}
		IV_x D IV_z \iff \forall u \in U_n: R^2_{Y \sim IV_x + \{u\}} > R^2_{Y \sim IV_z + \{u\}}
		\label{eq:cptdom}
	\end{equation}
	% fix symbolic logic here
	
	Hence, for all subsets of IVs $u$ in the pool of all possible $2^{p - 2}$ combinations of IVs not including  $IV_x$ and $IV_z$ (i.e., $U_n$), the $R^2$ that includes $IV_x$ must be greater than the $R^2$ that includes $IV_z$.
	...
	Each IV has a conditional dominance statistic for each number of IVs in the model. 
	Thus, with $p$ IVs, each IV will have $p$ conditional dominance statistics to compare.
	Extending on Table \ref{tab:exdom}, determining conditional dominance between $IV_x$ and $IV_z$ would involve all four different conditional dominance statistics as it outlined below in Table \ref{tab:excdl}.
	...
	\begin{table}[h!]
		...
		\begin{tabular}{ l | l l }
			...
			\hline
			& $(\Delta R^2_{Y \sim IV_x + \{IV_w\}})\frac{1}{3} + $ & $(\Delta R^2_{Y \sim IV_z + \{IV_w\}})\frac{1}{3} + $ \\
			Two IVs & $(\Delta R^2_{Y \sim IV_x + \{IV_v\}})\frac{1}{3} + $ & $(\Delta R^2_{Y \sim IV_z + \{IV_v\}})\frac{1}{3} + $ \\
			& $(\Delta R^2_{Y \sim IV_x + \{IV_z\}})\frac{1}{3}$ & $(\Delta R^2_{Y \sim IV_z + \{IV_x\}})\frac{1}{3}$ \\
			\hline
			& $(\Delta R^2_{Y \sim IV_x + \{IV_w + IV_v\}})\frac{1}{3} + $ & $(\Delta R^2_{Y \sim IV_z + \{IV_w + IV_v\}})\frac{1}{3} + $ \\
			Three IVs & $(\Delta R^2_{Y \sim IV_x + \{IV_w + IV_z\}})\frac{1}{3} + $ & $(\Delta R^2_{Y \sim IV_z + \{IV_w + IV_x\}})\frac{1}{3} + $ \\
	...
	Note that, unlike complete dominance designations that exclude $R^2$ subset values that include the IV against which the focal IV is being compared, conditional dominance designations are not IV pair-specific and the computation to produce the complete dominance statistics include all IVs.
	
	... ended here...
	
	The computations in Table \ref{tab:excdl} can be generalized
	
	\begin{equation}
		C^{i}_{IV_x} = \frac{\Sigma^{c}_{j} \Delta R^2_{IV_x + \{o_j\}}}{c}
		\label{eq:cdlst}
	\end{equation}
	...
	Conditional dominance is determined for $IV_x$ when $IV_x$ \emph{always} has a larger average incremental $R^2$ value compared to the compared to $IV_z$ across \emph{all} IV number-grouped averages/conditional dominance statistics.
		...
		\begin{equation}
		IV_x D_c IV_z \iff \bigcup^p_i C^{i}_{IV_x} > C^{i}_{IV_z}
		\label{eq:cdldom}
		\end{equation}
	% fix symbolic logic here
