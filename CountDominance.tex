...
\author{Anonymized-author\affilnum{1}}
\affiliation{\affilnum{1}Anoymized-affiliation}

\begin{abstract}
	Count variables are common in organizational science as an outcome. Count regression models (CRMs), such as Poisson regression, are recommended as methods to analyze count variables but can be challenging to interpret given their non-linear functional form. I recommend relative importance analysis as a method to use in interpreting CRM results. This work extends on past research by describing an approach to determining the importance of independent variables in CRMs using dominance analysis (DA). In this manuscript, I review DA as a relative importance method, recommend an $R^2$ to use with CRM-based DA, and outline the results of an analysis with simulated data that uses the recommended methodology. I also discuss the effect of differential exposure to the count generating probability across observations on count outcomes, how to correct for differential exposure, and the effect of differential exposure on relative importance determination. This work contributes to the literature by extending DA to CRMs, discussing the impact of differential exposure on CRM-based relative importance, and providing thoroughly documented example analysis that researchers can use to implementing the methodology in their research.
\end{abstract}
...
\section{Dominance Analysis: A Unique Relative Importance Analysis Method}
	...
	These methods, however informative in specific circumstances, are not considered to be useful as importance metirics as they fail to simultaneously account for the IV's bivariate and multivariate (i.e., controlling for other IVs) relationships with the DV \cite{johnson2004history}.
...
\section{Applying Dominance Analysis to Count Regression Models}
	...
	This section provided an extensive example of a PR model analysis using simulated data.
	The PR model described in this section interpreted model coefficients using IRRs and applied the recommended DA methodology using the $R^2_{DEV}$ to compute dominance statistics and designations.
	...
	In the section below, I shift the focus to a conceptual discussion of what exposure means for count DVs, how differential exposure is accommodated in CRMs, and offer an example of how differential exposure affects DA statistics and designations with another data analytic example.
	
\section{Additional Consideration for CRMs}

	CRMs are designed to model the semi-continuous and non-negative values on which count DVs take.
	Count DVs, in addition to being semi-continuous and non-negative, are also marked by an implied level of aggregation.
	Note that all the count DVs discussed in the introduction, as well as the simulated DV in the example above, aggregate a number of events over a specific period of time.	
	
	Count DVs as aggregated events is a common interpretation of a Poisson distributed variable. 
	...
	I will apply this transformation to a very simple CRM: an intercept only model that is fit with an offset term.
	...
	Finally, I use the property of logarithms that the difference between two logarithms is the logarithm of their quotient, thus producing $\ln \frac{Y}{X_{exposure}} = \beta_1$ or, when back translated to exponential form, $\frac{Y}{X_{exposure}} = e^{\beta_1}$.
	...
	As a result of the reduced unexplainable variation, inclusion of an offset term affects the values obtained by  $R^2_{DEV}$ and DA statistics on which they are based.
	
	In the sections below, I show the effect that differential exposure has on the results of CRMs by extending the analysis applied to the sport jacket production variable to two new variables that are contaminated with differential exposure.
	These two new variables are based on the same underlying causal model as the sport jacket production variable above but have been contaminated with two different kinds of differential exposure.
	...
	This section extends on the data analysis above by developing two additional sport jacket production variables that were observed during time periods with two different types of holidays.	
	...
	The focus in this extended analysis remains on evaluating the effect of the four tailoring shop characteristics in predicting stock jacket production from the same set of 6,780 tailoring shops. 
	The difference in the count variables produced below are that they were measured during two-week periods with different kinds of holidays.
	...
	These inconsistent leaves of absence resulted in each tailoring shop having different numbers of days in which no work was done.
	On days in which no work was done there was, of course, no exposure to the count generating probability.
	...
	Thus, the differential exposure process contaminating the sport jacket production variable due to taking a voluntary holiday is correlated with the tailoring shop characteristics.
	
	The means, standard deviations, and correlations of the two new sport jacket production variables with each of the tailoting shop IVs are reported in Table \ref{tab:dscEx}.
	...
	The two types of holidays described above have the effect of making fewer working days and thus will reduce the magnitude of the mean and standard deviations of Poisson distributed sport jacket production variables.	
	...
	By contrast, the tailoring shop characteristic-correlated voluntary holiday differential exposure should change the relative magnitude of the correlations consistent with the exposure process (described in the online supplement).
	...
	Voluntary holidays however do change the relative magnitudes (again, see online supplement for a discussion).
	...
	Both models are reported and compared to show the effect of the inclusion of the offset term on model results.
	
		\subsubsection{Random Differential Exposure}
		
	The two models estimated using religious holiday contaminated sport jacket production are reported in Table \ref{tab:unexp}.
	...
	The section below moves to the analysis using the correlated differential exposure mechanism introduced by taking voluntary holidays.
	...
	The two models estimated using voluntary holiday contaminated sport jacket production are reported in Table \ref{tab:exp}.
	...
	As compared to the random differential exposure results in Table \ref{tab:unexp}, the correlated differential exposure results in Table \ref{tab:exp} show clear differences in the patterns across models with and without an offset term.
	The correlated differential exposure effect in the model without an offset has changed the coefficients' values noticeably, shrinking all IRR values with the most substantial shrinkage seen for equipment reliability.
	By contrast, the model with the offset term obtains coefficient values that are, generally, more similar to the data under equal exposure especially for the equipment reliability and skill level IVs.
	This change in the relative magnitude of the coefficients in the model without an offset is the result of the IVs now having two ways in which to predict the sport jacket production outcome: the traditional path where the IV increases the probability of obtaining a sport jacket in a given day and, in addition, the exposure path where the IV increases the probability of any given day being a non-work day.
	...
	Taken together, these results help to illustrate the value of including the offset term for the accurate computation of DA designations and statistics when there is differential exposure in the count DV.
...
\section{Discussion}
	...
	Although a thorough evaluation of the potential bias introduced by omitting an offset term when differential exposure has contaminated the count DV was not undertaken in this work, I see future research focusing on this issue as applied to DA of potential value.
	Such reseach could be of value in helping researchers to identify situations when such bias is more or less likely to occur given their data.
	...
	This manuscript has provided a recommended methodology for extending DA to CRMs, offered extensive examples focusing on the interpretation of DA statistics and designations with simulated data, and provided recommendations for how to adjust a CRM-based DA when observations in the count DV may have different levels of exposure to the count generating probability.
	In combination, the conceptual discussion of DA, CRMs, and differential exposure, when paired with the empirical examples in this paper, will provide organizational and other behavioral scientists with the tools necessary for better understanding the results of CRMs they estimate and to more completely answer the research questions to which CRMs are applied.
