...
Conceptually, the underlying Normally distributed variable can be thought of as a latent propensity each tailoring shop has for making more garments. This garment propensity variable (called $GP$) is generated as linear combination of the four tailoring shop IVs. In devising the higher-order population coefficients, I thought it would be convenient to generate coefficients that, when combined with the higher-order population variances, would produce a specific $R^2$ under idealized conditions. In generating these coefficients, I intentionally ignored the higher-order population covariances to allow this section to be easier to follow and because I was not so much trying to attain a specific $R^2$ in the observed data but rather to have a reasonable, systematic approach to generating IV coefficient values.
...
This section describes the translation of the underlying Normal variable GP into the negative Binomial distributed OC variable.

The generation of the OC variable followed the same procedure as that to create SJ as it is created from the Normal cumulative probabilities of GP then translated to negative Binomial quantiles assuming a _size_ of .5 and _mu_ parameter of 1.
...
The results from Table S11 confirm expectations about the negative Binomial distributed variable's mean and standard deviation ($\sigma_{OC} = 1.757; \sigma^2_{OC} = 3.087$; . OC also produced similar correlations with the IVs as the GP variable on which it was based but smaller values than SJ. The further reduced correlation magnitudes among the IVs with OC was due to the effect of partly discretizing the GP variable into the OC variable paired with the strong skew introduced by the additional variance of the negative binomial

Figure Sc also shows the distribution of OC variable.
...
Data_Frame %>%
  ...
  tab_spanner(columns = c(ER, AS, SL, WE, SJ, OC), label = "Correlations") %>%
  tab_style(
    style = cell_borders(sides = "right"),
    locations = cells_body(columns = "Standard Deviation")
  ) %T>%
...
Data_Frame %>%
  ...
  bind_cols(
    Data_Frame %>% 
      ...
      select(-SJ, -GP, -`IV Name`, -OC, -SJ_RH, -RH)
  ) %>%
  gt() %>%
...
Data_Frame %>%
  ...
  bind_cols(
    Data_Frame %>% 
      ...
      select(-SJ, -GP, -`IV Name`, -OC, -SJ_RH, -RH, -SJ_VH, -VH)
  ) %>%
  gt() %>%
...
This section describes the model estimation and dominance analysis of the primary analysis models involving SJ and OC variables.
...
This section's focus is on obtaining all modeling results associated with the NBR using OC as DV.
...
IV_subset_names %>% 
  as_tibble() %>%
  bind_cols(OC_nb_subsets) %>%
  set_names(c("variable", "OC")) %>%
  gt(rowname_col = "variable") %>%
...
IV_subset_names %>% 
  ...
  bind_cols(SJ_pois_subsets) %>%
  bind_cols(OC_nb_subsets) %>%
  set_names(c("variable", "Poisson/SJ", "Negative Binomial/OC")) %>%
  gt(rowname_col = "variable") %>%
...
SJ_pois_DA %>% 
  ...
  bind_rows(tibble(Variable = "Overall", 
                   ...
                   `Negative Binomial/OC` = OC_nb_reg %>% r2_kullback(adjust = FALSE))) %>%
  gt(rowname_col = "Variable") %>%
